    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (currentLine.length !== headers.length) {
          setError(`Line ${i + 1} has an incorrect number of fields`);
          return;
        }
      ...
      if (results.length > 0) {
        setSuccess(
          `Successfully parsed ${results.length} student records. Review and submit to add to system.`
        );
      } else {
      ...
      console.error(err);
      setError("Error parsing CSV file. Please check format and try again.");
    }
  };
